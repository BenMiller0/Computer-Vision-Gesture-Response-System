...
def picamera2_pre_callback(request: CompletedRequest):
    ...
    # Print wrist coordinates for each detected person
    if keypoints is not None:
        for i, person in enumerate(keypoints):
            left_wrist = person[9]  # keypoint index for left wrist
            right_wrist = person[10]  # keypoint index for right wrist
            print(f"Person {i}: Left Wrist: {left_wrist[:2]}, Right Wrist: {right_wrist[:2]}")
